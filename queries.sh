...
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games losers ON teams.team_id=losers.opponent_id FULL JOIN games win ON teams.team_id=win.winner_id WHERE (win.year=2014 OR losers.year=2014) AND (win.round='Eighth-Final' OR losers.round='Eighth-Final') ORDER BY name ")"
